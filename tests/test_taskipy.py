...
class TaskipyTestCase(unittest.TestCase):
    ...
    def run_task(
        ...
        args = args or []
        options = options or []
        proc = self.start_taskipy_process(task, args=args, options=options, cwd=cwd)
    ...
    def start_taskipy_process(
        ...
        args = args or []
        options = options or []
        command = [executable_path] + options + [task] + args
        return subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=cwd)

    def create_test_dir_from_fixture(self, fixture_name: str):
        project_generator = GenerateProjectFromFixture(
            path.join("tests", "fixtures", fixture_name)
        )
...
class PassArgumentsTestCase(TaskipyTestCase):
    def test_running_task_with_positional_arguments(self):
        cwd = self.create_test_dir_from_fixture(
            "project_with_tasks_that_accept_arguments"
        )
        some_random_number = random.randint(1, 1000)
        exit_code, stdout, _ = self.run_task(
            "echo_number", args=[f"{some_random_number}"], cwd=cwd
        )
    ...
    def test_running_task_with_arguments_with_spaces(self):
        cwd = self.create_test_dir_from_fixture(
            "project_with_task_that_checks_args_passed_with_spaces"
        )
        name = "Roy Sommer"
        age = random.randrange(1, 100)
        exit_code, stdout, _ = self.run_task(
            "identify", args=["--full-name", name, "--age", f"{age}"], cwd=cwd
        )
    ...
    def test_running_task_arguments_not_passed_to_pre_hook(self):
        ...
        some_random_number = random.randint(1, 1000)
        exit_code, stdout, _ = self.run_task(
            "echo_on_prehook", args=[f"{some_random_number}"], cwd=cwd
        )
    ...
    def test_running_task_arguments_not_passed_to_post_hook(self):
        ...
        some_random_number = random.randint(1, 1000)
        exit_code, stdout, _ = self.run_task(
            "echo_on_posthook", args=[f"{some_random_number}"], cwd=cwd
        )
...
class ListTasksTestCase(TaskipyTestCase):
    ...
    def test_running_task_list_no_tasks(self):
        ...
        self.assertTerminalTextEqual(
            "no tasks found. create your first task by adding it to your pyproject.toml file under [tool.taskipy.tasks]",
            stdout.strip(),
        )
...
class RunTaskWithTimeTestCase(TaskipyTestCase):
    def test_running_task_execution_time_displayed(self):
        cwd = self.create_test_dir_with_py_project_toml(
            """
            [tool.taskipy.tasks]
            wait_one_second = "python -c 'import time; time.sleep(1)'"
        """
        )
        exit_code, stdout, _ = self.run_task("wait_one_second",options=['--time'], cwd=cwd)
        self.assertSubstr("ran wait_one_second in", stdout)
        self.assertEqual(exit_code, 0)

    def test_running_task_execution_time_displayed_with_shorthand(self):
        cwd = self.create_test_dir_with_py_project_toml(
            """
            [tool.taskipy.tasks]
            wait_one_second = "python -c 'import time; time.sleep(1)'"
        ...
        exit_code, stdout, _ = self.run_task("wait_one_second",options=['-t'], cwd=cwd)
        self.assertSubstr("ran wait_one_second in", stdout)
        self.assertEqual(exit_code, 0)

    def test_running_task_execution_time_within_expected_range(self):
        cwd = self.create_test_dir_with_py_project_toml(
            """
            [tool.taskipy.tasks]
            wait_one_second = "python -c 'import time; time.sleep(1)'"
        ...
        exit_code, _, _ = self.run_task("wait_one_second",options=['--time'], cwd=cwd)
        end_time = time.time()
...
class InterruptingTaskTestCase(TaskipyTestCase):
    ...
    def interrupt_task(self, process: subprocess.Popen):
        ...
        innermost_process = next(
            filter(lambda p: p.name().lower().startswith("python"), processes)
        )
    ...
    def test_handling_sigint_according_to_subprocess_if_it_handles_it_gracefully(self):
        cwd = self.create_test_dir_from_fixture(
            "project_with_tasks_that_handle_interrupts"
        )
        process = self.start_taskipy_process(
            "run_loop_with_interrupt_handling", cwd=cwd
        )
    ...
    def test_handling_sigint_according_to_subprocess_if_it_does_not_handle_it_gracefully(
        self,
    ):
        cwd = self.create_test_dir_from_fixture(
            "project_with_tasks_that_handle_interrupts"
        ...
        process = self.start_taskipy_process(
            "run_loop_without_interrupt_handling", cwd=cwd
        )
    ...
    def test_sigterm_should_be_sent_to_subprocess(self):
        cwd = self.create_test_dir_from_fixture(
            "project_with_tasks_that_handle_sigterm"
        )
...
class CustomRunnerTestCase(TaskipyTestCase):
    ...
    def test_running_command_with_custom_runner_fails_if_custom_runner_is_not_string(
        self,
    ):
        py_project_toml = """
            [tool.taskipy.settings]
        ...
        self.assertSubstr(
            "invalid value: runner is not a string. please check [tool.taskipy.settings.runner]",
            stdout,
        )
        self.assertEqual(exit_code, 1)
...
class TaskFromChildTestCase(TaskipyTestCase):
    def test_running_parent_pyproject_task_from_child_directory(self):
        ...
        _, stdout, _ = self.run_task(
            "print_current_dir_name", cwd=path.join(cwd, "child_without_pyproject")
        )
    ...
    def test_find_nearest_pyproject_from_child_directory(self):
        cwd = self.create_test_dir_from_fixture("project_with_tasks_from_child")
        _, stdout, _ = self.run_task(
            "hello", cwd=path.join(cwd, "child_with_pyproject")
        )
...
class RecursiveVariablesTestCase(TaskipyTestCase):
    ...
    def test_non_recursive_variables_cant_use_other_variables(self):
        ...
        exit_code, stdout, _ = self.run_task("echo", cwd=cwd)
        self.assertSubstr(
            "full_name_1: {first_name} {last_name} full_name_2: {first_name} {last_name}",
            stdout,
        )
        self.assertEqual(exit_code, 0)
